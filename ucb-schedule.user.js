...
if(UCBSE.term == "Fall")
	UCBSE.termTelebearsOracle = 'fa';
else
	UCBSE.termTelebearsOracle = 'sp';
UCBSE.termTelebearsOracle += UCBSE.year.substring(2, 4);
...
UCBSE.Course = function()
{
	...
	return {
		...
		parseLocn: function(str)
		{
			...
			if(temp != null)
			{
				days = str.match(/^[\s]*(M|Tu|W|Th|F|SA|Su){1,7}[\s]/);
				if(days)
					this.days = days[0];
		...
		fancyDays: function(days)
		{
			...
			if(days.match(/Su/) && !days.match(/SA/))
			{
				dayArr.push("--");
				dayArr.push("Su");
			}
			else if(days.match(/Su/))
				dayArr.push("Su");
...
UCBSE.css = (function()
{
	...
	css += ".col24, .col23, .col22, .col21, .col20, .col19 { padding:0 2px; }"
	...
	for(var i = 0, len = courseList.length; i < len; i++)
	{
		...
		if(prevCourseNum !== crs.getCourseNum() || prevDepartmentAbrevForCourseTitle !== crs.getDepartmentAbrev())
		{
			...
			if(prevDepartment == "COMPSCI")
				tableRows += '<a href="http://www-inst.eecs.berkeley.edu/~cs' + prevCourseNum.toLowerCase() + '/" target="_blank" alt="Course Webpage">' + prevCourseNum + '</a>';
			else if(prevDepartment == "EL ENG")
				tableRows += '<a href="http://www-inst.eecs.berkeley.edu/~ee' + prevCourseNum.toLowerCase() + '/" target="_blank" alt="Course Webpage">' + prevCourseNum + '</a>';
			else
...
UCBSE.controls = (function()
{
	...
	createToggleColumnElement(container, 24, "TelebearsOracle Link");
	container.appendChild(document.createElement("hr"));
